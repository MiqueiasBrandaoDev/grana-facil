-- Execute este script no painel do Supabase para criar as tabelas de administração
...
DROP POLICY IF EXISTS "Active admins can view admin_logs" ON public.admin_logs;
DROP POLICY IF EXISTS "Active admins can insert admin_logs" ON public.admin_logs;
DROP POLICY IF EXISTS "Active admins can view deploy_history" ON public.deploy_history;
DROP POLICY IF EXISTS "Super admins can insert deploy_history" ON public.deploy_history;
DROP POLICY IF EXISTS "Super admins can update deploy_history" ON public.deploy_history;
...
-- Insert the first super admin user (only if doesn't exist)
